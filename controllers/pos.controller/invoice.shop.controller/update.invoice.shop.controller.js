  ...
  try {
    ...
    upload(req, res, async function (err) {
      ...
      if (!req.file) {
        ...
        InvoiceShop.findByIdAndUpdate(id, req.body, {
...
async function uploadFile(req, res) {
  ...
  let fileMetaData = {
    ...
    parents: [process.env.GOOGLE_DRIVE_IMAGE_INVOICESHOP],
  };
  let media = {
  ...
  try {
    ...
    const id = req.params.id;
    InvoiceShop.findByIdAndUpdate(
      id,
      { ...req.body, invoice_image: response.data.id },
      { useFindAndModify: false }
    )
      .then((data) => {
        if (!data) {
