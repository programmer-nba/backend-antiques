const {
  ProductRef,
  validate,
} = require("../../../models/pos.models/product.reference.model");
...
exports.delete = async (req, res) => {
  const id = req.params.id;
  try {
    ProductRef.findByIdAndRemove(id, { useFindAndModify: false })
      .then((data) => {
        console.log(data);
        if (!data) {
          res.status(404).send({
...
exports.update = async (req, res) => {
  ...
  try {
    ...
    ProductRef.findByIdAndUpdate(id, req.body, { useFindAndModify: false })
      .then((data) => {
        console.log(data);
        if (!data) {
          res.status(404).send({
...
exports.create = async (req, res) => {
  ...
  try {
    ...
    const result = await new ProductRef({
      ...req.body,
    }).save();
    res.status(201).send({
...
exports.findByShopId = async (req, res) => {
  ...
  console.log(id);
  try {
    ProductRef.find({ ProductRef_shop_id: id })
      .then((data) => {
        if (!data)
          res
            .status(404)
