const {
  Advertising,
  validate,
} = require("../../../models/pos.models/advertising.image.model");
...
exports.delete = async (req, res) => {
  const id = req.params.id;
  try {
    Advertising.findByIdAndRemove(id, { useFindAndModify: false })
      .then((data) => {
        console.log(data);
        if (!data) {
          res.status(404).send({
...
exports.update = async (req, res) => {
  ...
  try {
    ...
    Advertising.findByIdAndUpdate(id, req.body, { useFindAndModify: false })
      .then((data) => {
        console.log(data);
        if (!data) {
          res.status(404).send({
...
exports.create = async (req, res) => {
  ...
  try {
    ...
    const result = await new Advertising({
      ...req.body,
    }).save();
    res.status(201).send({
