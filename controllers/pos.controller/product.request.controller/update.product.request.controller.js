...
exports.update = async (req, res) => {
  try {
    let upload = multer({ storage: storage }).single("productReq_image");
    upload(req, res, async function (err) {
      ...
      if (!req.file) {
        ...
        ProductReq.findByIdAndUpdate(id, req.body, {
...
async function uploadFile(req, res) {
  ...
  try {
    ...
    const id = req.params.id;
    ProductReq.findByIdAndUpdate(
      id,
      { ...req.body, productReq_image: response.data.id },
      { useFindAndModify: false }
    )
      .then((data) => {
        if (!data) {
