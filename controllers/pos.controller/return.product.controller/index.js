...
exports.delete = async (req, res) => {
  const id = req.params.id;
  try {
    ReturnProduct.findByIdAndRemove(id, { useFindAndModify: false })
      .then((data) => {
        console.log(data);
        if (!data) {
          res.status(404).send({
...
exports.update = async (req, res) => {
  ...
  try {
    ...
    ReturnProduct.findByIdAndUpdate(id, req.body, { useFindAndModify: false })
      .then((data) => {
        console.log(data);
        if (!data) {
          res.status(404).send({
...
exports.create = async (req, res) => {
  ...
  try {
    ...
    const result = await new ReturnProduct({
      ...req.body,
    }).save();
    res.status(201).send({
...
exports.findByShopId = async (req, res) => {
  ...
  console.log(id);
  try {
    ReturnProduct.find({ rtp_shop_id: id })
      .then((data) => {
        if (!data)
          res
            .status(404)
