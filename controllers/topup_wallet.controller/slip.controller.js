...
exports.update = async(req, res)=>{
    try{
        ...
        if(!(req.body.status !=='รอตรวจสอบ'||req.body.status!=='ตรวจสอบสำเร็จ'|| req.body.status !=='ยกเลิก')){
            return res.status(400).send({status: false, message: 'status : จะต้องเป็น รอตรวจสอบ, ตรวจสอบสำเร็จ, ยกเลิก เท่านั้น'})
        }
        if(req.body.status==='ยกเลิก' && !req.body.remark){
            return res.status(400).send({status: false, message : 'กรณียกเลิก จะต้องมี Remark หรือ เหตุผลที่ยกเลิก'})
        }
